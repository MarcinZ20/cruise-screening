...
const EditProfile = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const requestBody = {
      ...formData,
      languages: formData.languages.map((x) => x.id),
      knowledge_areas: formData.knowledge_areas.map((x) => x.id),
    };
  ...
  return (
    <Base>
      <div className="max-w-3xl mx-auto p-6 bg-white dark:bg-background-darker border border-gray-200 dark:border-none rounded-lg shadow-lg my-12">
        ...
        {message && <p className="text-center text-red-default mb-4">{message}</p>}
        <form onSubmit={handleSubmit} className="space-y-4">
          <input
            ...
            placeholder="Email"
            className="w-full p-2 border rounded-md dark:bg-background-dark dark:border-gray-400 dark:text-text-dimmed"
          />
          <input
            type="text"
            name="first_name"
          ...
          <div className="flex items-center space-x-2">
            <input
              ...
              onChange={handleChange}
            />
            <label className="dark:text-text-dimmed">
              Allow Logging (This helps improve the quality of the search
              engine.)
            </label>
          </div>
          <input
          ...
          <div className="flex space-x-4">
            <button
              type="submit"
              className="bg-orange-default text-white px-4 py-2 rounded-md hover:bg-orange-700 transition"
            >
              Save Changes
            </button>
